...
router.get('/:id/kpis', async (req, res, next) => {
  try {
    const kpis = await getKPIs(req.params.id);
    if (!kpis) {
...
router.get('/:id/stops-by-robot', async (req, res, next) => {
  try {
    const data = await getStopsByRobot(req.params.id);
    res.json({ data });
  } catch (err) {
    next(err);
  }
...
router.get('/:id/reason-distribution', async (req, res, next) => {
  try {
    ...
    const data = await getReasonDistribution(req.params.id, level as 'l1' | 'l2' | 'l3');
    res.json({ data });
  } catch (err) {
    next(err);
  }
...
router.get('/:id/heatmap', async (req, res, next) => {
  try {
    const data = await getHeatmapData(req.params.id);
    res.json({ data });
  } catch (err) {
    next(err);
  }
